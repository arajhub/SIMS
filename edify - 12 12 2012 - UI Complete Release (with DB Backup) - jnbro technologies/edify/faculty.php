...
<div id="pagecontent">
  <div>
    ...
    <li></li>
      ...
      <?php
								if($username){
									$teacherListQuery = "SELECT * FROM qualifications WHERE designation = 'teacher'";
									$teacherListResult = mysql_query($teacherListQuery);
									$teacherListRows = mysql_num_rows($teacherListResult);
									if($teacherListRows > 0) {
									
										for($i = 0;$i < $teacherListRows;$i++) {
											$teacherArray = mysql_fetch_array($teacherListResult);
										
											
											echo '<div style=" width:245px; height:80px; float:left;">'.smallprofilebox($teacherArray['username'],$teacherArray['desgn']).'</div>';
										}
									}
							...
							<script language="javascript">
								window.location.replace('http://edify.atrisesolutions.com/');
							</script>
							<?php
								}
							?>
