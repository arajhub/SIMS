...
<div id="pageheader" class="headergrey">
				<p class="title">Upload Files - Results
...
<?php
						...
						while(list($key,$value) = each($_FILES['uploadedfile']['name'])) {
							if(!empty($value)) {
								...
								$filename = str_replace('"','\"',$filename);
								$insertQuery = "INSERT INTO files_uploaded (file_name,uploaded_by,approved_flag) VALUES ('".$filename."','".$username."','false')";
								mysql_query($insertQuery) or die ('Cannot insert into database');
								mysql_query("SHOW ERRORS");
								$target_path = "uploads/";
